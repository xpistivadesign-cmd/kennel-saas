...
export async function GET() {
  try {
    ...
    const { data: appointments } = await supabase.from("veterinary_appointments").select("*").eq("user_id", user.id).order("start_at", { ascending: true });
    const { data: documents } = await supabase.from("veterinary_documents").select("*").eq("user_id", user.id).order("created_at", { descending: true });
...
export async function POST(request: Request) {
  try {
    ...
    if (actionType === "create_appointment") {
      const payload = {
        user_id: user.id,
        dog_id: formData.get("dog_id")?.toString(),
    ...
    if (actionType === "upload_document") {
      const dog_id = formData.get("dog_id")?.toString();
      ...
      if (medicalFile && medicalFile.size > 0) {
        ...
        const { data: uploadData, error: uploadErr } = await supabase.storage
          .from("logos") // Meglévő bucket felhasználása
