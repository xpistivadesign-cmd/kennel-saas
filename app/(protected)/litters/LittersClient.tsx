      ...
      <div className="flex border-b border-zinc-800 gap-2">
        <button 
          ...
          className={`p-2 uppercase font-bold ${
            tab === "directory" ? "text-amber-400 border-b-2 border-amber-500" : "text-zinc-500"
          }`}
        >
          📂 Directory
        </button>
        <button 
          ...
          className={`p-2 uppercase font-bold ${
            tab === "planner" ? "text-amber-400 border-b-2 border-amber-500" : "text-zinc-500"
          }`}
        >
          🎯 Planner
        </button>
        {litter && (
          <button 
            ...
            className={`p-2 uppercase font-bold ${
              tab === "litter-profile" ? "text-amber-400 border-b-2 border-amber-500" : "text-zinc-500"
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {litters.map((l: any) => (
            <div key={l.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded flex flex-col justify-between">
              <button 
                onClick={() => { setSelId(l.id); setTab("litter-profile"); }} 
                className="text-left block w-full"
              >
                <div className="flex justify-between items-center">
                  <h2 className="text-lg font-bold text-amber-400">"{l.letter}" alom</h2>
                  <span className="p-1 bg-zinc-800 text-zinc-400 rounded text-[10px]">{l.status}</span>
                </div>
                <p className="text-zinc-400 mt-1">
                  Apa: {l.sire_name || "N/A"} • Anya: {l.dam_name || "N/A"}
                </p>
                <p className="text-zinc-500 font-mono mt-1">Dátum: {l.birth_date || "Nincs megadva"}</p>
              </button>
              <div className="flex gap-2 mt-3 justify-end">
                {l.status === "Tervezett" && (
                  <button onClick={() => onBorn(l.id)} className="bg-emerald-600 px-2 py-1 rounded font-bold">
                    🎉 Megszületett
                  </button>
                )}
                <button onClick={() => onDel(l.id, l.letter)} className="bg-zinc-800 text-red-400 px-2 py-1 rounded font-bold">
                  🗑️ Törlés
                </button>
              </div>
            </div>
      ...
      {tab === "planner" && (
        <form action={createLitterAction} className="bg-zinc-900 p-4 rounded border border-zinc-800 max-w-md space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-zinc-500 block mb-1">Alom Betűje (Litter Letter)</label>
              <input name="letter" required placeholder="Pl. B" className="w-full p-2 bg-black border border-zinc-800 rounded" />
            </div>
            <div>
              <label className="text-zinc-500 block mb-1">Date</label>
              <input name="birth_date" type="date" className="w-full p-2 bg-black border border-zinc-800 rounded" />
            </div>
          </div>
          <div>
            ...
            {sireType === "other" && (
              <input name="sire_name" placeholder="External sire name" className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded mt-1" />
            )}
          </div>
          <div>
            ...
            {damType === "other" && (
              <input name="dam_name" placeholder="External dam name" className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded mt-1" />
            )}
          </div>
          <div>
            <label className="text-zinc-500 block mb-1">Státusz</label>
            <select name="status" className="w-full p-2 bg-black border border-zinc-800 rounded">
